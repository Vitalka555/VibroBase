...
bool copyfile::cppSlot()
{
    ...
    QString destinationFile = settings.value("PathToPhoto", QDir::currentPath() + QString("/foto/")).toString() +"/"+ fileInfo.fileName();
...
bool copyfile::cppSlot2()
{
    ...
    QSettings settings(path, QSettings::IniFormat);
    settings.beginGroup("PathToFiles");
    QString destinationFile = settings.value("PathToShema", QDir::currentPath() + QString("/foto/")).toString() +"/"+ fileInfo.fileName();
    qDebug() << destinationFile;
    settings.endGroup();
    path1.replace("file:///", "");
    qDebug() << "path1 = " << path1;
